...
from codara.core.models import SessionStatus, ProviderType, Workspace, Session
...
def _run_uvicorn_reload(app, host: str, port: int, project_root: Path) -> None:
    uvicorn.run(
        ...
        port=port,
        reload=True,
        reload_dirs=[str(project_root / "src" / "codara")],
    )
...
def serve(host, port, build_ui, dev_mode):
    """Start the Codara Gateway server (serves API + Dashboard UI).
    ...
    click.echo(f"Codara Gateway: http://{host}:{port}")
...
def dev(host, port):
    """Start Codara in development mode with hot-reload.
    ...
    backend_proc = subprocess.Popen(
        [
            "uvicorn", "codara.gateway.app:app",
            "--host", host,
            "--port", str(port),
            "--reload",
            "--reload-dir", str(project_root / "src" / "codara"),
        ],
        cwd=project_root,
    )
...
@cli.group()
def workspace():
    """Manage user-facing Codara workspaces."""
    pass
