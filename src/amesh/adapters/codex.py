...
from amesh.core.models import Session, Message, TurnResult
from amesh.adapters.base import ProviderAdapter, CliRuntimeMixin
from amesh.adapters.cli_monitor import communicate_with_stall_detection, terminate_process
from amesh.cli_run_store import CliRunStore
from amesh.config import get_provider_default_model, get_settings
from amesh.core.models import ProviderType
from amesh.telemetry import current_trace_context
...
class CodexAdapter(ProviderAdapter, CliRuntimeMixin):
    ...
    async def list_models(self, settings: Any) -> dict[str, Any]:
        ...
        if cached is not None:
            return cached
        from amesh.config import get_provider_default_model
        from amesh.core.models import ProviderType
