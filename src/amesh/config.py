"""Centralized configuration for the Ameshe program."""
...
DEFAULT_CONFIG_FILENAME = "amesh.toml"
...
class Settings(BaseSettings):
    ...
    # Database
    database_path: str = Field(default="amesh.db", validation_alias="UAG_DATABASE_PATH")
...
def get_config_dir() -> Path:
    """Resolve the central Ameshe config directory (shared vault & settings)."""
    return Path("~/.amesh").expanduser()
...
def _resolve_path_like_settings(config_values: Dict[str, Any], config_path: Path) -> Dict[str, Any]:
    ...
    # the user places `amesh.toml` at repo root or under `configs/`.
